...
from detect_hidden_sites import interpolate_bare_earth, residual_relief, detect_anomalies

def parse_args():
    parser = argparse.ArgumentParser(description="Preview each step of the hidden site detection pipeline")
    parser.add_argument("--bbox", nargs=4, type=float, metavar=("xmin", "ymin", "xmax", "ymax"),
                      required=False, help="Longitude/Latitude bounds")
    ...
    parser.add_argument("--resolution", type=float, default=0.0002695,
                       help="Interpolation grid step in degrees (~30 m)")
    parser.add_argument("--sigma", type=int, default=2,
                       help="Gaussian σ for residual-relief smoothing")
    parser.add_argument("--outdir", default="preview_outputs",
                       help="Directory to save preview visualizations")
    parser.add_argument("--max-points", type=int, default=1000000000000000,
...
def visualize_copernicus_dem(
    ...
    rgb = ls.shade(dem_for_ls, cmap=cmap, blend_mode="soft",
                   vert_exag=vert_exag, vmin=vmin, vmax=vmax)
    ...
    if bare:
        ...
        if bbox:
            ...
            ax.plot([xmin_b, xmax_b, xmax_b, xmin_b, xmin_b],
                    [ymin_b, ymin_b, ymax_b, ymax_b, ymin_b],
                    color="red", lw=.7, label="AOI")
            ax.legend()

        ax.set_xlabel("Easting (m)" if project else "Longitude")
        ax.set_ylabel("Northing (m)" if project else "Latitude")
        ax.set_title("Copernicus DEM (Web-Mercator)" if project
                     else "Copernicus DEM (WGS-84)")

    # ─── 4. save PNG ---------------------------------------------------------------
    outdir = Path(outdir); outdir.mkdir(parents=True, exist_ok=True)
    base = "copernicus_dem_hillshade" if bare else "copernicus_dem_map"
    if project: base += "_3857"
...
def visualize_gedi_points(points, bbox, outdir):
    ...
    scatter = ax.scatter(lons, lats, c=elevs, cmap='viridis', 
...
def visualize_residual_relief(xi, yi, zi, dem_path, outdir):
    ...
    extent = [np.min(xi), np.max(xi), np.min(yi), np.max(yi)]
    im = ax.imshow(rrm_valid, extent=extent, origin='upper', 
...
def visualize_anomalies(anomalies, rrm, xi, yi, sigma, geojson_path, outdir):
    ...
    if not anomalies.empty:
        ...
        anomaly_points = ax.scatter(
            anomalies.geometry.x, anomalies.geometry.y,
    ...
    if geojson_path:
        try:
            ...
            truth_gdf.plot(ax=ax, color='none', edgecolor='lime', 
                         linewidth=2, zorder=5, label='Known Sites')
            ax.legend()
        except Exception as e:
...
def main():
    ...
    if args.interactive:
        create_interactive_map(points, anomalies, args.bbox, outdir)
